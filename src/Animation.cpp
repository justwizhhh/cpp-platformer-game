...
Animation::Animation(const std::string& texture_path, float max_speed, int max_frames)
{
  if (!texture.loadFromFile(texture_path))
  {
    std::cout << " Texture from " << texture_path << " unable to be found!" << std::endl;
  }
  else
  {
...
//
// Stops playing the currently used animation
//
void Animation::stop()
{
  is_playing = false;
